...
public class AdminDataRepository {
    ...
    public void ensureSeedData() {
        ...
        if (count("sys_tenant") == 0) {
            jdbc.update("INSERT INTO sys_tenant(tenant_id,tenant_name,contact_user,contact_phone,package_name,status,remark) VALUES('default','默认租户','管理员','','基础套餐','enabled','系统默认租户')");
        }
        if (count("sys_role") == 0) {
            ...
            jdbc.update("INSERT INTO sys_role(tenant_id,role_key,role_name,status,menu_keys,remark) VALUES('default','tenant_admin','租户管理员','enabled','dashboard,selection,reports','租户配置管理员')");
            jdbc.update("INSERT INTO sys_role(tenant_id,role_key,role_name,status,menu_keys,remark) VALUES('default','operator','运营人员','enabled','dashboard,reports','选品运营人员')");
        }
        if (count("sys_user") == 0) {
        ...
        if (count("sys_dict_type") == 0) {
            jdbc.update("INSERT INTO sys_dict_type(tenant_id,dict_name,dict_type,status,remark) VALUES('default','系统状态','sys_normal_disable','enabled','通用启停状态')");
            jdbc.update("INSERT INTO sys_dict_type(tenant_id,dict_name,dict_type,status,remark) VALUES('default','市场区域','market_region','enabled','选品市场区域')");
        ...
        if (count("market_configs") == 0) {
            jdbc.update("INSERT INTO market_configs(tenant_id,market_key,market_name,region,enabled,note) VALUES('default','jp','日本市场','日本',true,'已接入 demo 趋势数据，可配置真实 TikTok/Amazon JP 数据源')");
            jdbc.update("INSERT INTO market_configs(tenant_id,market_key,market_name,region,enabled,note) VALUES('default','us','美国市场','美国',false,'即将接入 Amazon US / TikTok US 数据源')");
            jdbc.update("INSERT INTO market_configs(tenant_id,market_key,market_name,region,enabled,note) VALUES('default','sea','东南亚市场','东南亚',false,'即将接入 TikTok Shop SEA / Shopee / Lazada 数据源')");
        }
        if (count("category_configs") == 0) for (String c : List.of("玩具","家居","美妆","宠物","数码","户外","母婴","汽车","厨房","文具","服饰","健康")) jdbc.update("INSERT INTO category_configs(tenant_id,category_name,market_key,enabled,keywords,note) VALUES('default',?,'jp',true,?,'默认日本市场品类')", c, c);
    ...
    public boolean validateLogin(String username, String password) { ensureSeedData(); List<String> values = jdbc.query("SELECT password FROM sys_user WHERE username=? AND status='enabled'", (rs,n)->rs.getString(1), username); return !values.isEmpty() && values.get(0).equals(password); }
    public String tenantOf(String username) { return userInfo(username).tenantId; }
    private SysUserInfo userInfo(String username) { ensureSeedData(); return jdbc.queryForObject("SELECT username,nickname,tenant_id,role_key FROM sys_user WHERE username=?", (rs,n)->new SysUserInfo(rs.getString("username"),rs.getString("nickname"),rs.getString("tenant_id"),rs.getString("role_key")), username); }
    private List<String> permissions(String tenantId, String roleKey) { List<String> menuKeys = jdbc.query("SELECT menu_keys FROM sys_role WHERE tenant_id=? AND role_key=?", (rs,n)->rs.getString(1), tenantId, roleKey); if (menuKeys.isEmpty() || "*".equals(menuKeys.get(0))) return List.of("*:*:*"); return Arrays.stream(menuKeys.get(0).split(",")).map(String::trim).filter(s->!s.isBlank()).collect(Collectors.toList()); }
    private record SysUserInfo(String username,String nickname,String tenantId,String roleKey) {}

    public List<SysTenant> tenants(){ ensureSeedData(); return jdbc.query("SELECT * FROM sys_tenant ORDER BY id", (rs,n)->new SysTenant(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("tenant_name"),rs.getString("contact_user"),rs.getString("contact_phone"),rs.getString("package_name"),rs.getString("status"),rs.getString("remark"))); }
    ...
    public List<AdminUser> users(String tenantId){ ensureSeedData(); String sql="*".equals(tenantId)?"SELECT * FROM sys_user ORDER BY id":"SELECT * FROM sys_user WHERE tenant_id='"+tenantId+"' ORDER BY id"; return jdbc.query(sql, (rs,n)->new AdminUser(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("username"),"",rs.getString("nickname"),rs.getString("role_key"),rs.getString("status"),rs.getString("email"),rs.getString("phone"))); }
    public AdminUser saveUser(AdminUser u){ ensureSeedData(); if(u.id()>0) jdbc.update("UPDATE sys_user SET tenant_id=?,username=?,nickname=?,role_key=?,status=?,email=?,phone=? WHERE id=?", def(u.tenantId()),u.username(),u.nickname(),u.roleKey(),u.status(),u.email(),u.phone(),u.id()); else jdbc.update("INSERT INTO sys_user(tenant_id,username,password,nickname,role_key,status,email,phone) VALUES(?,?,?,?,?,?,?,?)", def(u.tenantId()),u.username(), blank(u.password(),"admin"), u.nickname(),u.roleKey(),u.status(),u.email(),u.phone()); return users("*").stream().filter(x->x.username().equals(u.username())).findFirst().orElse(users("*").get(0)); }
    public void deleteUser(long id){ jdbc.update("DELETE FROM sys_user WHERE id=? AND username <> 'admin'", id); }

    public List<AdminRole> roles(String tenantId){ ensureSeedData(); String sql="*".equals(tenantId)?"SELECT * FROM sys_role ORDER BY id":"SELECT * FROM sys_role WHERE tenant_id='"+tenantId+"' ORDER BY id"; return jdbc.query(sql, (rs,n)->new AdminRole(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("role_key"),rs.getString("role_name"),rs.getString("status"),split(rs.getString("menu_keys")),rs.getString("remark"))); }
    ...
    public List<AdminMenu> menus(){ ensureSeedData(); List<AdminMenu> flat=jdbc.query("SELECT * FROM sys_menu ORDER BY sort_order,id", (rs,n)->new AdminMenu(rs.getLong("id"),rs.getLong("parent_id"),rs.getString("menu_key"),rs.getString("title"),rs.getString("icon"),rs.getString("path"),rs.getString("component"),rs.getString("permission"),rs.getInt("sort_order"),rs.getString("status"),new ArrayList<>())); return tree(flat,0); }
    public List<AdminMenu> flatMenus(){ ensureSeedData(); return jdbc.query("SELECT * FROM sys_menu ORDER BY sort_order,id", (rs,n)->new AdminMenu(rs.getLong("id"),rs.getLong("parent_id"),rs.getString("menu_key"),rs.getString("title"),rs.getString("icon"),rs.getString("path"),rs.getString("component"),rs.getString("permission"),rs.getInt("sort_order"),rs.getString("status"),List.of())); }
    ...
    public List<SysDictData> dictData(String tenantId){ ensureSeedData(); String sql="*".equals(tenantId)?"SELECT * FROM sys_dict_data ORDER BY dict_type,sort_order,id":"SELECT * FROM sys_dict_data WHERE tenant_id='"+tenantId+"' ORDER BY dict_type,sort_order,id"; return jdbc.query(sql,(rs,n)->new SysDictData(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("dict_type"),rs.getString("dict_label"),rs.getString("dict_value"),rs.getInt("sort_order"),rs.getString("status"),rs.getString("remark"))); }
    ...
    public List<SysConfig> configs(String tenantId){ ensureSeedData(); String sql="*".equals(tenantId)?"SELECT * FROM sys_config ORDER BY id":"SELECT * FROM sys_config WHERE tenant_id='"+tenantId+"' ORDER BY id"; return jdbc.query(sql,(rs,n)->new SysConfig(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("config_name"),rs.getString("config_key"),rs.getString("config_value"),rs.getBoolean("system_builtin"),rs.getString("remark"))); }
    ...
    public List<MarketConfig> markets(String tenantId){ ensureSeedData(); String sql="*".equals(tenantId)?"SELECT * FROM market_configs ORDER BY id":"SELECT * FROM market_configs WHERE tenant_id='"+tenantId+"' ORDER BY id"; return jdbc.query(sql,(rs,n)->new MarketConfig(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("market_key"),rs.getString("market_name"),rs.getString("region"),rs.getBoolean("enabled"),rs.getString("note"))); }
    ...
    public List<CategoryConfig> categories(String tenantId){ ensureSeedData(); String sql="*".equals(tenantId)?"SELECT * FROM category_configs ORDER BY id":"SELECT * FROM category_configs WHERE tenant_id='"+tenantId+"' ORDER BY id"; return jdbc.query(sql,(rs,n)->new CategoryConfig(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("category_name"),rs.getString("market_key"),rs.getBoolean("enabled"),rs.getString("keywords"),rs.getString("note"))); }
    ...
    public List<SysOperLog> operLogs(String tenantId){ ensureSeedData(); String sql="*".equals(tenantId)?"SELECT * FROM sys_oper_log ORDER BY id DESC LIMIT 200":"SELECT * FROM sys_oper_log WHERE tenant_id='"+tenantId+"' ORDER BY id DESC LIMIT 200"; return jdbc.query(sql,(rs,n)->new SysOperLog(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("username"),rs.getString("module"),rs.getString("action"),rs.getString("method"),rs.getString("status"),rs.getString("message"),instant(rs.getTimestamp("created_at")))); }
    public List<SysLoginLog> loginLogs(String tenantId){ ensureSeedData(); String sql="*".equals(tenantId)?"SELECT * FROM sys_login_log ORDER BY id DESC LIMIT 200":"SELECT * FROM sys_login_log WHERE tenant_id='"+tenantId+"' ORDER BY id DESC LIMIT 200"; return jdbc.query(sql,(rs,n)->new SysLoginLog(rs.getLong("id"),rs.getString("tenant_id"),rs.getString("username"),rs.getString("ipaddr"),rs.getString("status"),rs.getString("message"),instant(rs.getTimestamp("created_at")))); }
